...
int Messages_Load_Messages(int numberOfmessages, char * nameFileMessage) {
	...
	if (mf==NULL) {
	   ...
	   ComputerSystem_DebugMessage(NO_TIMED_MESSAGE,62,POWERON,nameFileMessage);
	   return -1;
	  }
		...
		number=strtok(lineRead,",");
 		if ((number!=NULL) && (number[0]!='/') && (number[0]!='\n') && (number[0]!='\r')) {
	    	...
	    	if (rc==0){
					...
					ComputerSystem_DebugMessage(NO_TIMED_MESSAGE,60,POWERON,lineNumber,nameFileMessage);
					continue;
				}
	  		...
	  		if (text==NULL){
					...
					ComputerSystem_DebugMessage(NO_TIMED_MESSAGE,61,POWERON,lineNumber,nameFileMessage);
					continue;
				}
			rc=	Messages_Set(msgNumber,text);
			switch (rc) {
				case -1:
					ComputerSystem_DebugMessage(NO_TIMED_MESSAGE,65,POWERON);
					break;
				case -2:
					ComputerSystem_DebugMessage(NO_TIMED_MESSAGE,66,POWERON,msgNumber);
					break;
				default:
					numberOfmessages++;
				}
  ...
  ComputerSystem_DebugMessage(NO_TIMED_MESSAGE,63,POWERON,(numberOfmessages-initialMessage), nameFileMessage);
